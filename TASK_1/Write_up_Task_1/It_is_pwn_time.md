...
Vì có sự khác biệt thời gian khi chạy source code nên phải brute time để tìm flag, bìa khá dễ nhưng hơi lâu.
$\to$ Solution:
...
def gen_RSA_params(N, e):
    ...
    return (p, q, n)
print(t)
for x in tqdm(range(1000000), desc = "Generating keys"):
    
    random.seed(t)

    p, q, n_1 = gen_RSA_params(KEY_SIZE, e)
    phi = (p-1)*(q-1)
