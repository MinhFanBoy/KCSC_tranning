...
Ban đầu mình thấy cấp của G khá nhỏ, là số smooth prime theo factordb và private cũng nhỏ nên mình nghĩ tới việc tính polig-hellman. Mà poglig hellman có độ phức tạp là $O(\sqrt{p})$ nên cũng không khả thi lắm, chứ chưa nói tới việc brute cháy máy như Dũng nên ta hướng tới thuật toán tương tự có độ phức tạp nhỏ hơn là :

![image](https://github.com/MinhFanBoy/KCSC_tranning/assets/145200520/2fe3f554-2348-444d-9d7d-ca4ec2925adc)

:)


Cụ thể hơn ta sử dụng tính chất a = b mod (p * q) thì a = b mod(p) và a = b mod(q) từ đó ta có thể dùng CRT để tìm ra nghiệm đúng. Từ đó ta tính log rời rạc của các số nguyên tố có được từ việc tách số nguyên tố, rồi sử dụng CRT, từ đó bài toán thay vì $O(\sqrt(q ^ n))$ chỉ còn $O(n * \sqrt{q}$ từ đó làm giảm đáng kể thời gian.

Note: Yêu cầu cấp của E phải đủ nhỏ và có thể dễ dàng phân tích thành số nguyên tố
...
Từ đó, ta dễ dàng có được các thông số cần thiết để giải nốt phần còn lại.
...
15423694994465574149 92209717447332837440641806732517921920015580446111641942522142444036785043977
```

phần còn lại làm giống như các bài trước.
...
secret = 92209717447332837440641806732517921920015580446111641942522142444036785043977
tmp = {'iv': 'ceb34a8c174d77136455971f08641cc5', 'encrypted_flag': 'b503bf04df71cfbd3f464aec2083e9b79c825803a4d4a43697889ad29eb75453'}
...
flag = decrypt_flag(secret, bytes.fromhex(tmp['iv']), bytes.fromhex(tmp['encrypted_flag']))
print(flag)
